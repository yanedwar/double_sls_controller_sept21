...
int main(int argc, char **argv){
    ...
    ros::NodeHandle nh;
    ros::Subscriber state_sub = nh.subscribe<mavros_msgs::State> ("/uav0/mavros/state", 10, stateCb);    

    ros::Publisher local_pos_pub = nh.advertise<geometry_msgs::PoseStamped> ("/uav0/mavros/setpoint_position/local", 10);
    ros::Publisher attitude_setpoint_pub = nh.advertise<mavros_msgs::AttitudeTarget> ("/uav0/mavros/setpoint_raw/attitude", 10);
    ros::Publisher test_setpoint_pub = nh.advertise<mavros_msgs::AttitudeTarget> ("/uav0/double_sls_controller/setpoint_raw/attitude", 10);
    ros::Publisher dsls_state_pub = nh.advertise<double_sls_controller::DSlsState> ("/uav0/double_sls_controller/dsls_state", 10);
    ros::Publisher dea_state_pub = nh.advertise<double_sls_controller::DEAState> ("/uav0/double_sls_controller/dea_state", 10);
    ros::Publisher dea_force_1_pub = nh.advertise<geometry_msgs::Vector3Stamped> ("/uav0/double_sls_controller/dea_force_1", 10);
    ros::Publisher dea_force_2_pub = nh.advertise<geometry_msgs::Vector3Stamped> ("/uav0/double_sls_controller/dea_force_2", 10);
    ...
    while(ros::ok()){
        ...
        // ROS_INFO_STREAM("q1:" << pend0_q.x*pend0_q.x + pend0_q.y*pend0_q.y + pend0_q.z*pend0_q.z);
...
void applyDEAController(
    ...
    getAttitudeTarget(F1, attitude_dea);
